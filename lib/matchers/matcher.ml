    ...
    let between p from until =
      (fun s ->
         begin
           ...
           (*(if _is_alphanum until then (skip _whitespace) else return ()) >>= fun _ ->*)

           (* do not consider until valid unless current char/token is like whitespace or non-alphanum delim or hole *)
