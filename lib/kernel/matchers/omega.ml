...
module Make (Language : Types.Language.S) (Meta : Metasyntax.S) (Ext : External.S) = struct
  module rec Matcher : Types.Matcher.S = struct
    ...
    let implicit_equals_satisfied environment identifier range matched =
      ...
      | Some existing_value ->
        let Range.{ match_start = { offset; _ }; _ } =
          Option.value_exn (Environment.lookup_range environment identifier)
        in
        if offset = range.match_start.offset then (* case when already present from rest parser *)
          Some environment
    ...
    let record_match_context pos_before pos_after rule =
      ...
      let extract_matched_text source { offset = match_start; _ } { offset = match_end; _ } =
        if debug then
          Format.printf "Attempt slice start %d end %d on %S@." match_start match_end source;
        ...
        if rewrite then (
          let result, _ =
            Template.substitute
              ...
              (Template.parse !rewrite_template)
      ...
      | Some rule ->
        ...
        let metasyntax =
          Metasyntax.{ syntax = Meta.syntax; identifier = Meta.identifier; aliases = Meta.aliases }
        in
        ...
        (match new_env with
         ...
         | Some env ->
           ...
           (let result, _ =
            ...
            Buffer.add_string actual result);
           matches_ref
             := { match_context with environment = !current_environment_ref } :: !matches_ref)
    ...
    let seq p_list = List.fold p_list ~init:(return (Unit, "")) ~f:( *> )
    ...
    let convert ?left_delimiter ?right_delimiter (p_list : (production * 'a) t list)
      ...
      let add_match user_state identifier p =
        ...
        p
        >>= fun value ->
        let m = { offset; identifier; text = value } in
      ...
      List.fold (List.rev p_list) ~init:[] ~f:(fun acc p ->
        ...
        | Ok (Hole { sort; identifier; dimension; _ }, user_state) ->
          (match sort with
           ...
           | Non_space ->
             let non_space =
               ([ Omega_parser_helper.skip space1; Omega_parser_helper.skip reserved_parsers ]
               |> choice
               |> not_followed_by)
               *> any_char
               >>| Char.to_string
             ...
             let rest =
               ...
               | _ -> return () >>= fun () -> seq acc >>= fun r -> return r
             in
    ...
    let sequence_chain' ?left_delimiter ?right_delimiter p_list =
      convert ?left_delimiter ?right_delimiter p_list |> seq

    let generate_pure_spaces_parser _ignored = spaces1 >>= fun s1 -> r acc (Template_string s1)
    ...
    let hole_parser sort dimension : (production * 'a) t t =
      ...
      let skip_signal hole =
        Omega_parser_helper.skip (string "_signal_hole") >>| fun () -> Hole hole, acc
      in
    ...
    let generate_hole_for_literal dimension ~contents ~left_delimiter ~right_delimiter () =
      let literal_holes =
        choice
        @@ List.map Template.Matching.hole_parsers ~f:(fun (kind, _) -> hole_parser kind dimension)
      in
      let reserved_holes = List.map reserved_holes ~f:Omega_parser_helper.skip in
      let other =
        Omega_parser_helper.(
          up_to @@ choice [ spaces1 *> return (); choice reserved_holes *> return () ]
          >>| String.of_char_list)
      in
      let parser =
        many
    ...
    let loose_whitespace g =
      ...
      let rec aux acc i = function
        ...
        | hd :: tl when i = 0 ->
          ...
          aux (acc @ [ p ]) (i + 1) tl
        | hd :: tl -> aux (acc @ [ hd ]) (i + 1) tl
      in
      match g with
      | [] -> g
      | [ p ] -> [ loose_constants `Right (loose_constants `Left p) ]
      | l -> aux [] 0 l
    ...
    let general_parser_generator rule : (production * 'a) t t =
      ...
      in
      let other =
        many1 (not_followed_by reserved_parsers *> any_char)
        >>| String.of_char_list
      ...
      | p_list ->
        ...
        (match !configuration_ref.match_kind with
         | Exact ->
           ...
           implicit_equals_match_satisfied := true;
           (* reset *)
           ...
           let match_one =
             ...
             let start_pos =
               if String.length value = 0 then
                 start_pos
               (*offset + 1 this may not matter, if we correct for the whole match conext *)
               else
                 start_pos
             in
             (if String.length value = 0 then
    ...
    let all
      ...
      let rec aux_all ?configuration ?(nested = false) ~template ~source () =
        matches_ref := [];
        if String.is_empty template && String.is_empty source then
          [ trivial ]
        else (
          match first_is_broken template source rule with
          | Ok _ | Error _ ->
                    ...
                    let range =
                      { match_start =
                          { m.range.match_start with
                            offset = ms.offset + m.range.match_start.offset
                          }
                      ; match_end =
      ...
      let result =
        ...
        else (* Don't reverse the list for non-nested matches--it matters for rewriting. *)
          aux_all ?configuration ~nested ~template ~source:original_source ()
      in
