...
let run ?(configuration = configuration) source match_template rewrite_template =
  Matchers.Generic.first ~configuration match_template source
  |> function
  | Ok result ->
...
let%expect_test "contextual_matching" =
  ...
  let match_template = {|memcpy(:[1], :[2], :[3])|} in
  let rewrite_template = {|:[1]|} in
  run_all (module Generic) source match_template rewrite_template;
...
let%expect_test "trivial_empty_case" =
  ...
  run_match (module Generic) source match_template;
  [%expect_exact
...
let%expect_test "test_top_level_hole_stops_at_newline_false_implies_default_generic" =
  ...
  run_all (module Generic) ~configuration source match_template rewrite_template;
  [%expect_exact
    {|
line
line
...
let%expect_test "test_top_level_hole_crosses_newlines_for_html_by_default" =
  ...
  run_all (module Html) ~configuration source match_template rewrite_template;
  [%expect_exact {|
